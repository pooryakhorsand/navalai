# resistance/yacht_three/views.py
import base64
import gzip
import hashlib
import json
...
from .engine import yacht_three_calculate
...
def _build_series_payload(
    ...
    n = int(np.asarray(froude_series).shape[0])
    ...
    if response_format == "compact":
        ...
        for key, arr in arrays.items():
            payload[key] = _encode_f32_base64(np.asarray(arr)[idx])
    else:
        payload = {}
        for key, arr in arrays.items():
            selected = np.asarray(np.asarray(arr)[idx], dtype=np.float64)
...
class YachtThreeView(View):
    """
    Optimized yacht three-parameter (L, B, T only) endpoint.

    POST /resistance/yacht-three/

    Predicts the remaining hull parameters (lcb, Pc, displacement) from
    L, B, T via YachtCalculation, then solves the resistance curve —
    same solver used by the full Yacht Resistance Predictor.
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            result = yacht_three_calculate(
                L=parsed["L"],
                B=parsed["B"],
                T=parsed["T"],
            )

            yacht_params = result["parameters"]
            ...
            total_arr = np.asarray(res["total_series"])
            max_total_idx = int(np.argmax(total_arr))

            metrics = {
                "max_total_resistance": _safe_float(total_arr[max_total_idx]),
                "froude_at_max_total": _safe_float(
                    np.asarray(res["froude_series"])[max_total_idx]
                ),
                "max_residuary_resistance": _safe_float(
                    np.max(np.asarray(res["residuary_series"]))
                ),
            }
            ...
            output_payload = {
                ...
                "message": "Yacht Three calculations completed successfully.",
            }
            ...
            return _json_error(
                message=f"Yacht Three calculation fault: {str(exc)}",
                status=500,
            )