...
from django.shortcuts import render  # اضافه شده برای رندر قالب فرانت‌اند
from django.utils.decorators import method_decorator
from django.views import View
from django.views.decorators.csrf import csrf_exempt

from .engine import ZigzagModel, _COEF_ORDER
...
def _parse_payload(data):
    ...
    T = _as_float(data, "T", default=600.0)
    t0 = _as_float(data, "t0", default=10.0)
    ui_initial_deg = _as_float(data, "ui_initial", default=0.0)
    h = _as_float(data, "h", default=0.1)
    delta1_deg = _as_float(data, "delta1", default=20.0)
    ...
    return {
        ...
        "T": T,
        "t0": t0,
        "ui_initial": math.radians(ui_initial_deg),
        "x_initial": x_initial,
...
class ZigzagSimulationView(View):
    """
    Optimized 20/20 zigzag maneuver simulation endpoint.
    Handles both GET for UI rendering and POST for numerical calculations.
    """

    http_method_names = ["get", "post", "options"]

    def get(self, request, *args, **kwargs):
        # رندر مستقیم تمپلیت فرانت‌اند زیکزاک زمانی که آدرس در مرورگر باز می‌شود
        return render(request, "maneuver/zigzag.html")
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            (
                ...
                delta_series,
                max_yaw,
            ) = model.simulate_zigzag(
                T=parsed["T"],
                t0=parsed["t0"],
                ui_initial=parsed["ui_initial"],
            )
            ...
            output_payload = {
                ...
                "data": {
                    "metrics": {
                        "max_yaw_deg": _safe_float(max_yaw),
                    },
                    "time_series": time_series_payload,
                    "series_meta": series_meta,
                },
                "message": "Zigzag maneuver simulation completed successfully.",
            }
            ...
            return _json_error(
                message=f"Zigzag simulation failed: {str(exc)}",
                status=500,
            )