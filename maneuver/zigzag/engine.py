...
def _simulate_zigzag_core(x0, h, N, t0, ui_initial, U0, L, m, Iz, xG, coef):
    (
        Xudot, Yvdot, Nvdot, Xu, Yrdot, Nrdot, Xuu, Yv, Nv,
        Xuuu, Yr, Nr, Xvv, Yvvv, Nvvv, Xrr, Yvvr, Nvvr,
        Xdd, Yvu, Nvu, Xudd, Yru, Nru, Xrv, Yd, Nd, Xvd,
    ...
    for i in range(N + 1):
        ...
        psi_deg = x[5] * 180.0 / np.pi
        r_val = x[2]

        if time >= t0:
            if round(time) == round(t0):
                if abs(ui_initial) < 1e-6:
                    rudder = 20.0 * np.pi / 180.0
                else:
                    rudder = ui_initial
            else:
                if psi_deg >= 20.0 and r_val > 0.0:
                    rudder = -20.0 * np.pi / 180.0
                elif psi_deg <= -20.0 and r_val < 0.0:
                    rudder = 20.0 * np.pi / 180.0
...
class ZigzagModel:
    ...
    def simulate_zigzag(self, T=600.0, t0=10.0, ui_initial=0.0):
        N = int(round(float(T) / self.h))
        if N < 1:
            N = 1

        xout = _simulate_zigzag_core(
            self.x_initial,
            self.h,
            N,
            float(t0),
        ...
        U_series = xout[:, 7]
        delta_series = xout[:, 8] * (180.0 / np.pi)

        max_yaw = np.max(np.abs(psi_series))
