/**
 * app_turning.js
 * NavalAI — Turning Circle Maneuver Solver
 *
 * Optimized for:
 * - lazy Plotly loading
 * - lazy KaTeX loading
...
const API_URL = ASSETS.turningApi || '/maneuver/turning/';
...
const SLIDER_DEFS = [
  ...
  { id: 'sl-ui', key: 'ui', disp: 'val-ui', fmt: v => v.toFixed(0) + '°' },
...
let state = { U0: 7.7175, ui: -35, t0: 100, T: 700 };
let simCache = null;
...
let shipModel, trajLine, trajLineGeom;
...
function initToolbarEvents() {
  ...
  on('btn-download-img', 'click', function () {
    ...
    a.download = 'NavalAI_TurningCircle.png';
...
function updateHUD() {
  ...
  if (hu) hu.textContent = state.ui.toFixed(0) + '°';
}
...
async function runSimulation() {
  ...
  try {
    ...
    console.error('NavalAI Turning simulation error:', err);

    setStatus('err', 'ERROR');

    ['t-advance', 't-transfer', 't-tactical'].forEach(function (id) {
      const el = byId(id);
      if (el) {
        el.textContent = 'ERR';
...
function updateTelemetry() {
  ...
  setT('t-advance',  fmt(m.advance_val,  'm'));
  setT('t-transfer', fmt(m.transfer_val, 'm'));
  setT('t-tactical', fmt(m.tactical_val, 'm'));
...
function renderPlot() {
  ...
  if (mode === 'trajectory') {
    ...
    layout.title = { text: 'Surge Velocity (u) vs Time', font: { size: 13, color: '#94a3b8' } };
    layout.xaxis.title = 't [s]';
    layout.yaxis.title = 'u [m/s]';
  }
...
function renderTables() {
  ...
  function fmtMet(v) {
    return (v === null || v === undefined || isNaN(v)) ? '—' : Number(v).toFixed(2) + ' m';
  }
  ...
  target.innerHTML =
    '<div class="tbl-block" style="margin-bottom:1rem;">' +
      '<div class="tbl-title">IMO Turning Circle Metrics</div>' +
      '<table class="data-table"><thead><tr><th>Metric</th><th>Value</th></tr></thead><tbody>' +
        '<tr><td>Advance</td><td>' + fmtMet(m.advance_val) + '</td></tr>' +
        '<tr><td>Transfer</td><td>' + fmtMet(m.transfer_val) + '</td></tr>' +
        '<tr><td>Tactical Diameter</td><td>' + fmtMet(m.tactical_val) + '</td></tr>' +
      '</tbody></table>' +
    '</div>' +
    '<div class="tbl-block">' +
      ...
      '<table class="data-table"><thead><tr>' +
        ...
        '<th>u [m/s]</th><th>v [m/s]</th><th>r [°/s]</th><th>ψ [°]</th>' +
...
function exportCSV() {
  ...
  let csv = 'NavalAI - Turning Circle Maneuver Report\n\n';
  ...
  csv += 'METRICS\n';
  csv += 'Advance_m,' + (m.advance_val == null ? '' : m.advance_val) + '\n';
  csv += 'Transfer_m,' + (m.transfer_val == null ? '' : m.transfer_val) + '\n';
  csv += 'Tactical_m,' + (m.tactical_val == null ? '' : m.tactical_val) + '\n\n';

  csv += 'TIME SERIES\nt_s,x_m,y_m,U_mps,u_mps,v_mps,r_degps,psi_deg\n';
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = 'TurningCircle_Report.csv';
  ...
  if (frameDelta >= FRAME_INTERVAL) {
    if (simCache && shipModel && trajLineGeom) {
      ...
      if (phaseHud) {
        ...
        phaseHud.textContent = (realT < state.t0) ? 'PRE-RUDDER' : 'TURNING';
      }
    }
