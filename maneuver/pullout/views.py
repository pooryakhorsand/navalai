...
from .engine import PulloutModel, _COEF_ORDER
...
def _build_time_series_payload(
    ...
    arrays = {
        "t_series": t_series,
        "yaw_rate_positive": r_pos,
        "yaw_rate_negative": r_neg,
        "heading_positive": psi_pos,
        "heading_negative": psi_neg,
        "rudder_positive": delta_pos,
        "rudder_negative": delta_neg,
        "x_positive": x_pos,
        "y_positive": y_pos,
        "x_negative": x_neg,
        "y_negative": y_neg,
...
class PulloutSimulationView(View):
    """
    Optimized Pullout maneuver simulation endpoint.
    POST /maneuver/pullout/
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            output_payload = {
                ...
                "data": {
                    "metrics": {
                        "residual_yaw_rate_positive": _safe_float(residual_r_pos),
                        "residual_yaw_rate_negative": _safe_float(residual_r_neg),
                        "directional_stable": bool(abs(residual_r_pos) < 0.05 and abs(residual_r_neg) < 0.05),
                    },
                    "time_series": time_series_payload,
                    "series_meta": series_meta,
                },
                "message": "Pullout maneuver simulation completed successfully.",
            }
