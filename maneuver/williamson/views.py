...
from .engine import WilliamsonTurnModel, _COEF_ORDER
...
def _parse_payload(data):
    ...
    delta1_deg = _as_float(data, "delta1", default=35.0)
    ...
    if Tmax > 7200:
        raise ValueError(
            "Tmax is too large for this VPS profile. Maximum allowed Tmax is 7200 s."
        )
    ...
    output_stride = _as_int(
        data, "output_stride", default=1, min_value=1, max_value=500
    )
...
def _build_time_series_payload(response_format, output_stride, series_dict):
    n = int(series_dict["t_series"].shape[0])
    ...
    if response_format == "compact":
        payload = {"_format": "f32-base64-v1", "_length": returned_n}
...
class WilliamsonTurnSimulationView(View):
    """
    Optimized Williamson Turn simulation endpoint.

    POST /williamson_turn/
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            model = WilliamsonTurnModel(
                **parsed["params"],
                x0=parsed["x_initial"],
            )

            results = model.simulate_williamson(Tmax=parsed["Tmax"])

            series_dict = {
                "t_series": results["t_series"],
                "u_series": results["u_series"],
                "v_series": results["v_series"],
                "r_series": results["r_series"],
                "x_series": results["x_series"],
                "y_series": results["y_series"],
                "psi_series": results["psi_series"],
                "U_series": results["U_series"],
                "delta_actual_deg": results["delta_actual_deg"],
                "rudder_cmd_deg": results["rudder_cmd_deg"],
            }
            ...
            output_payload = {
                ...
                "data": {
                    "metrics": {
                        "cross_track_error_m": _safe_float(
                            results["metrics"]["cross_track_error_m"]
                        ),
                        "heading_error_deg": _safe_float(
                            results["metrics"]["heading_error_deg"]
                        ),
                        "max_yaw_deg": _safe_float(
                            results["metrics"]["max_yaw_deg"]
                        ),
                    },
                    "maneuver_info": {
                        "t1_60deg": _safe_float(
                            results["maneuver_info"]["t1_60deg"]
                        ),
                        "t2_minus130deg": _safe_float(
                            results["maneuver_info"]["t2_minus130deg"]
                        ),
                        "t_opp_heading": _safe_float(
                            results["maneuver_info"]["t_opp_heading"]
                        ),
                    },
                    "time_series": time_series_payload,
                    "series_meta": series_meta,
                },
                "message": "Williamson Turn simulation completed successfully.",
            }
